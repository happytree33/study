...
<form name="prosearch" id="prosearch" action="/biz/search" method="post" onsubmit="return false;">
	...
	<div class="unified_search">
		<div id="uni_srch_header" class="uni_srch_header">
			<div class="uni_srch_bar">
				<div class="tit">
					<h1><spring:message code="srch.unified.search" text="" /></h1>
				</div>
				<div class="logo">
					<img src="http://design7.naonsoft.com//resources/biz/search/img/logo_naon.png" alt="">
				</div>
...
<!-- 						<input id="srch_input" name="" type="text" title="검색어" placeholder="검색어" autocomplete="off" class="input_txt"> -->
							...
							<a href="#srch_suggest" class="srch_arw">
								<span><spring:message code="srch.text.autocomplete.open" text="" /></span>
							</a>
							<button type="button" title="검색" class="btn btn_ico" onClick="javascript:goSearch();">
								<i class="ico ico_srch"><span><spring:message code="srch.text.search" text="" /></span></i>
							</button>
						</div>
					</fieldset>
...
<!-- 							<div id="recomm_kwd" onclick="hideLayer('recomm_kwd');showLayer('suggest_kwd');" class="recomm_kwd" style="display:none;"> -->
							<div id="recomm_kwd" class="recomm_kwd" style="display:none;">
								<div class="kwd_tit">
									...
									<div class="optn">
										<a href="#" onClick="removeAllMyKeyword();" class="del_all"><spring:message code="srch.text.all.del" text="" /></a>
									</div>
								</div>
...
<!-- 									</ul> -->
				...
				<div class="srch_option">
					<div class="btn_drop detail_drop">
						<button type="button" data-toggle="dropdown" class="btn drop_tgl" onClick="changeDetailQuery()">
							<spring:message code="srch.text.detail.search" text="" />
						</button>
						<div class="drop_box unhide">
							...
							<div class="basic_input">
								...
								<label class="guide_txt"><input id="or_check" type="checkbox" value="" class="input_chk" onclick="printText()">
									<spring:message code="srch.text.include.contents" text="" />
								</label>
							</div>
							<div class="more_input">
							    ...
							    <div class="guide_txt">
							    	<spring:message code="srch.text.divide.word" text="" />
							    </div>
							</div>
							<div class="btn_area">
								<button type="button" class="btn btn_pri" onClick="detailSearch()"><strong>
									<spring:message code="srch.text.search" text="" /></strong>
								</button>
								<button type="button" class="btn" onClick="init()">
									<spring:message code="srch.text.init" text="" />
								</button>
							</div>
						</div>
					</div>
					<label class="re_srch">
						<input name="" type="checkbox" value="" class="input_chk"  id="reChk" onclick="javascript:resultReSearch();">
						<spring:message code="srch.text.requery" text="" />
					</label>
				</div>
			</div>
			<div class="uni_srch_nav">
				<div class="srch_nav">
					<ul class="nav nav_pills">
						<li class="active" id="TOTAL">
							<a href="#none"  onclick="goIndexSearch('TOTAL');">
							<spring:message code="srch.unified.search" text="" />
								<span class="count">${searchResult.totalCount}</span>
							</a>
						</li>
						<c:forEach var="indexList" items="${searchResult.indexTabList}" varStatus="status">
							...
							<c:choose>
								<c:when test="${count < 9}">
									<li id="indexlist${count}" class="<c:if test="${fn:contains(searchResult.index, indexList.indexName)}">active</c:if>">
										<a href="#none"  onclick="goIndexSearch('<c:out value="${indexList.indexName}" />');">
											<c:out value="${indexList.indexKorName}" />
											<span class="count"><c:out value="${indexList.indexTotalCount}" /></span>
										</a>
									</li>					
								</c:when>  
								<c:otherwise>
						...
						<li class="more">
						<div class="btn_drop">
							<button type="button" data-toggle="dropdown" class="btn drop_tgl">
								<spring:message code="srch.unified.more" text="" /><span class="caret"></span>
							</button>
							<ul class="drop_menu">
								<c:forEach var="indexList" items="${searchResult.indexTabList}" varStatus="status">
									<c:set var="count" value="${status.count}" />
		...
		<div id="uni_srch_content" class="uni_srch_content">
			<div class="area_main">
				<div class="result_top">
					...
					<div class="fr">
						<c:if test="${searchResult.index ne 'TOTAL' and searchResult.thisIndexCount ne '0'}"> 
						<!-- 더보기 눌렀을때만 보이게  -->
							<div class="combobox">
								<button type="button" data-toggle="dropdown" title="정렬순서" class="select_value"></button>
								<ul class="drop_menu pull_right">
									<li>
										<input name="a" id="a2" type="radio" value="SCORE" class="input_rdo" onClick="javascript:goSorting('SCORE/DESC');" <c:if test="${searchResult.sort eq 'SCORE/DESC'}">checked</c:if>>
										<label for="a2"><spring:message code="srch.text.rank" text="" /></label>
									</li>
									<li>
										<input name="a" id="a1" type="radio" value="doc_reg_dt" class="input_rdo" onClick="javascript:goSorting('doc_reg_dt');" <c:if test="${searchResult.sort eq 'doc_reg_dt'}">checked</c:if>>
										<label for="a1"><spring:message code="srch.text.date" text="" /></label>
									</li>
								</ul>						
							</div>
							<div class="combobox num_slt">
								<button type="button" data-toggle="dropdown" title="목록개수" class="select_value"></button>
								<ul class="drop_menu pull_right">
									<li><input name="lnum" id="lnum1" type="radio" value="" class="input_rdo" onClick="javascript:showCount('10');" <c:if test="${searchResult.indexsize eq 10}">checked</c:if>><label for="lnum1">10</label></li>
									<li><input name="lnum" id="lnum2" type="radio" value="" class="input_rdo" onClick="javascript:showCount('15');" <c:if test="${searchResult.indexsize eq 15}">checked</c:if>><label for="lnum2">15</label></li>
									<li><input name="lnum" id="lnum3" type="radio" value="" class="input_rdo" onClick="javascript:showCount('20');" <c:if test="${searchResult.indexsize eq 20}">checked</c:if>><label for="lnum3">20</label></li>
									<li><input name="lnum" id="lnum4" type="radio" value="" class="input_rdo" onClick="javascript:showCount('30');" <c:if test="${searchResult.indexsize eq 30}">checked</c:if>><label for="lnum4">30</label></li>
								</ul>
							</div>
						</c:if>				
					</div>
				</div>				
				<c:if test="${searchResult.totalCount eq '0' or (searchResult.index ne 'TOTAL' and searchResult.thisIndexCount eq '0')}">
				...
				<!-- 페이징 -->                                                     		                                      
				<c:if test="${searchResult.index ne 'TOTAL'}"> 
               		<div class="pagination">  
						<ul>
                     		<c:if test="${pageResult.startPageNo > 0}">
                        		<li <c:if test="${searchResult.pageNo eq 1}">class="disabled"</c:if>>
                           	 		<a class="page_frst" href="#" onClick="movePage('${pageResult.firstPageNo}');"><span>First</span></a>
                           		</li>
                           		<li>
                           			<a class="page_prev" href="#" onClick="movePage('${pageResult.prevPageNo}');"><span>Prev</span></a>
                           		</li>
	                           	...
	                           	<li>
	                           		<a href="#" class="page_next" onClick="movePage('${pageResult.nextPageNo}');"><span>Next</span></a>
	                           	</li>
	                           	<li <c:if test="${searchResult.pageNo eq pageResult.finalPageNo}">class="disabled"</c:if>>
	                           		<a href="#" class="page_last" onClick="movePage('${pageResult.finalPageNo}');"><span>Last</span></a>
	                       		</li>
                      		</c:if>
                   		</ul>
                	</div>
				...
				<div class="aside_cont">
					<div class="uni_srch_optn">
						<div class="optn_box">
							...
							<div class="cont">
								<ul class="rdo_ul">
									<li class="<c:if test="${'TOTAL' eq searchResult.range || '' eq searchResult.range }">active</c:if>">
										<label><input name="ra" type="radio" value="" class="input_rdo" checked onClick="setDate('TOTAL')">
										<spring:message code="srch.index.name.ALL" text="" /></label>
									</li>
									<li class="<c:if test="${'W' eq searchResult.range}">active</c:if>">
										<label><input name="ra" type="radio" value="" class="input_rdo" onClick="setDate('W')">
										<spring:message code="srch.text.range.week" text="" /></label>
									</li>
									<li class="<c:if test="${'M' eq searchResult.range}">active</c:if>">
										<label><input name="ra" type="radio" value="" class="input_rdo" onClick="setDate('M')">
										<spring:message code="srch.text.range.month" text="" /></label>
									</li>
									<li class="<c:if test="${'3M' eq searchResult.range}">active</c:if>">
										<label><input name="ra" type="radio" value="" class="input_rdo" onClick="setDate('3M')">
										<spring:message code="srch.text.range.3month" text="" /></label>
									</li>
									<li class="<c:if test="${'6M' eq searchResult.range}">active</c:if>">
										<label><input name="ra" type="radio" value="" class="input_rdo" onClick="setDate('6M')">
										<spring:message code="srch.text.range.6month" text="" /></label>
									</li>
						...
						<div class="optn_box">
							...
							<div class="cont">
								<ul class="chk_ul">
									<li class="<c:if test="${fn:indexOf(searchResult.searchField, 'TOTAL') gt -1}">active</c:if>">
										<label><input name="ca" type="checkbox" <c:if test="${fn:indexOf(searchResult.searchField, 'TOTAL') gt -1}">checked</c:if> value="TOTAL" class="input_chk" onClick="`Field('TOTAL');">
										<spring:message code="srch.index.name.ALL" text="" /></label>
									</li>
									<li class="<c:if test="${fn:indexOf(searchResult.searchField, 'title') gt -1}">active</c:if>">
										<label><input name="ca" type="checkbox" <c:if test="${fn:indexOf(searchResult.searchField, 'title') gt -1}">checked</c:if> value="title" class="input_chk"  onClick="goSearchField('title');">
										<spring:message code="srch.text.title" text="" /></label>
									</li>
									<li class="<c:if test="${fn:indexOf(searchResult.searchField, 'content') gt -1}">active</c:if>">
										<label><input name="ca" type="checkbox" <c:if test="${fn:indexOf(searchResult.searchField, 'content') gt -1}">checked</c:if> value="content" class="input_chk" onClick="goSearchField('content');">
										<spring:message code="srch.text.content" text="" /></label>
									</li>
									<li class="<c:if test="${fn:indexOf(searchResult.searchField, 'owner') gt -1}">active</c:if>">
										<label><input name="ca" type="checkbox" <c:if test="${fn:indexOf(searchResult.searchField, 'owner') gt -1}">checked</c:if>  value="owner" class="input_chk" onClick="goSearchField('owner');">
										<spring:message code="srch.text.writer" text="" /></label>
									</li>
									<li class="<c:if test="${fn:indexOf(searchResult.searchField, 'attach') gt -1}">active</c:if>">
										<label><input name="ca" type="checkbox"  <c:if test="${fn:indexOf(searchResult.searchField, 'attach') gt -1}">checked</c:if> value="attach" class="input_chk" onClick="goSearchField('attach');">
										<spring:message code="srch.text.attach" text="" /></label>
									</li>
								</ul>
							</div>
						</div>
